...
const SH_CLIENTES_ARCHIVO = "ClientesArchivados"; // nueva hoja de archivo
...
function setup() {
  ...
  // Crear la hoja de archivo si no existe
  ...
  if (confSheet.getLastRow() < 2) {
    ...
    confSheet.appendRow(["costoSinEtiquetaNormal", 1510]); // 1850 - 340
    confSheet.appendRow(["costoConEtiquetaLupulada", 1950]);
    confSheet.appendRow(["costoSinEtiquetaLupulada", 1610]); // 1950 - 340
    ...
    // Costos por litro de barriles
    ESTILOS_BARRIL.forEach(e => {
      if (e !== "VACIO") confSheet.appendRow(["costoLitro_" + e, 1600]); // Valor estimado, podés editarlo en la planilla
    });
  }
...
function accionSyncGeneral() {
      ...
      ventas: ventasObjs
        ...
        .map(function (v) {
          return {
            ...
            totalCobrado: Number(v.TotalCobrado) || 0,
            costo: Number(v.Costo) || 0,
            comision: Number(v.Comision) || 0,
            paraProfeta: Number(v.ParaProfeta) || 0,
  ...
  ventasObjs.forEach(function (v) {
    const monto = Number(v.TotalCobrado) || 0;
    const metodo = String(v.MetodoPago || "").toLowerCase().trim();
    const paraProfetaVenta = Number(v.ParaProfeta) || 0;
    if (metodo) {
...
function accionClientesTodos() { 
  const c = filaAObjetos(getSheet(SH_CLIENTES)); 
  return { clientesTodos: c.map(function(r){ return {nombre: r.Nombre, deuda: Number(r.Deuda)||0, pagado: Number(r.Pagado)||0}; }) }; 
}
...
function accionBorrarDeudaCliente(n) { 
  ...
  if(f){
    // mover a archivo antes de borrar
...
function accionBorrarVenta(d) { 
  ...
  for(let i=o.length-1;i>=0;i--){ 
    const v=o[i]; 
    if(String(v.Vendedor)===String(d.vendedor||"")&&String(v.Fecha)===String(d.fecha||"")&&String(v.Cliente||"Consumidor Final")==
    =String(d.cliente||"Consumidor Final")&&Number(v.TotalCobrado)===Number(d.totalCobrado||0)){ 
      f=v; break; 
    } 
  } 
  if(!f) return; 
...
// FIX: RESETEAR DEUDA SI SE SALDA
function upsertDeudaCliente(n, dd, dp) { 
  ...
  if(f){ 
    ...
    if (np >= nd) { 
      // si se salda, MOVEMOS el registro a la hoja de archivo y lo borramos de clientes
      const archive = getSheet(SH_CLIENTES_ARCHIVO);
...
// DEPÓSITO DE BARRIL <-> CARTERA DE DEUDORES
// El depósito/seña de un barril se suma a la Deuda del cliente (misma
// cuenta que sus ventas fiadas), así aparece en la cartera y se puede
// cobrar/cancelar total o parcialmente igual que cualquier otra deuda.
// No pasa por SH_PAGOS (no es venta ni cobro real) para no inflar
// efectivo/transferencia ingresados.
...
// Cancela (total o parcialmente) el depósito de un barril: suma "Pagado"
// para ese cliente, reduciendo su deuda, sin tocar SH_PAGOS.
